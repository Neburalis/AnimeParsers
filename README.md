...
# Допустимые значения студий и жанров для фильтра доступны без экземпляра.
studios = AnixartParser.get_studios()
genres = AnixartParser.get_genres()
```

### Статус реализации Anixart

Сейчас `AnixartParser` умеет:

- искать релизы и возвращать группу связанных релизов из расширенного поиска;
- фильтровать каталог по жанрам, студиям, годам, статусу и другим параметрам Anixart;
- получать карточку релиза, расписание, случайный релиз и связанные релизы;
- получать доступные озвучки, источники и серии;
- возвращать ссылки на внешние плееры;
- предоставлять списки жанров и студий для фильтра.

Получение самого видео пока не реализовано. `get_player_urls()` возвращает ссылки на внешние плееры из ответа Anixart, но не преобразует их в прямые HLS- или MP4-ссылки и не скачивает видео. Для этого потребуются отдельные парсеры внешних видеопровайдеров.

Каждый элемент результата `get_player_urls()` содержит ключи `type_id`, `type_name`, `source_id`, `source_name`, `episode_position`, `episode_name`, `iframe`, `quality` и `url`.

`get_studios()` и `get_genres()` возвращают новые изменяемые списки допустимых значений для параметров `studio` и `genres`. Списки взяты из ресурсов официального приложения Anixart 8.5.2 и встроены в библиотеку. Отдельного API для них нет, поэтому изменения со стороны Anixart потребуют обновления библиотеки.

### Фильтрация каталога Anixart

`filter()` обращается к отдельному каталожному endpoint Anixart. Он не выполняет поиск по названию: для этого используйте `search()`. Все аргументы фильтра именованные.
...
result = parser.filter(
    ...
    types=[9],
    age_ratings=[4],
    sort=AnixartParser.SORT_GRADE,
)

for release in result["content"]:
    print(release["id"], release.get("title_ru") or release.get("title"))
```

| Параметр | По умолчанию | Назначение | Допустимые значения |
|---|---:|---|---|
| `page` | `0` | Номер страницы | Целое число от `0` |
| `extended` | `True` | Запрашивать расширенные данные релизов | `True` или `False` |
| `category_id` | `None` | Категория релиза | `1` — сериал; `2` — полнометражный фильм; `3` — OVA; `4` — дорама |
| `country` | `None` | Страна производства | Непустая строка; приложение Anixart предлагает `"Япония"` и `"Китай"` |
| `start_year` | `None` | Нижняя граница года выпуска | Целый год; если указан `end_year`, значение не должно быть больше него |
| `end_year` | `None` | Верхняя граница года выпуска | Целый год; если указан `start_year`, значение не должно быть меньше него |
| `episode_duration_from` | `None` | Минимальная длительность серии в минутах | Целое число от `0`; не больше `episode_duration_to`, если заданы обе границы |
| `episode_duration_to` | `None` | Максимальная длительность серии в минутах | Целое число от `0`; не меньше `episode_duration_from`, если заданы обе границы |
| `episodes_from` | `None` | Минимальное количество серий | Целое число от `0`; не больше `episodes_to`, если заданы обе границы |
| `episodes_to` | `None` | Максимальное количество серий | Целое число от `0`; не меньше `episodes_from`, если заданы обе границы |
| `is_genres_exclude_mode_enabled` | `False` | Исключать релизы с указанными жанрами вместо их включения | `True` или `False` |
| `season` | `None` | Сезон выхода | `1` — зима; `2` — весна; `3` — лето; `4` — осень |
| `source` | `None` | Первоисточник | Непустая строка: `"Оригинал"`, `"Манга"`, `"Веб-манга"`, `"Енкома"`, `"Ранобэ"`, `"Новелла"`, `"Веб-новелла"`, `"Игра"`, `"Визуальная новелла"`, `"Карточная игра"`, `"Книга"`, `"Книга с картинками"`, `"Музыка"`, `"Радио"`, `"Более одного"` или `"Другое"` |
| `status_id` | `None` | Статус выхода | `1` — вышел; `2` — выходит; `3` — анонс |
| `studio` | `None` | Студия | Одно из значений `AnixartParser.get_studios()`, например `"MAPPA"`, `"Bones"` или `"Kyoto Animation"` |
| `sort` | `SORT_DATE_UPDATE` | Порядок выдачи | `SORT_DATE_UPDATE` (`0`) — по дате добавления; `SORT_GRADE` (`1`) — по рейтингу; `SORT_YEAR` (`2`) — по годам; `SORT_POPULAR` (`3`) — по популярности |
| `genres` | `None` | Жанры для включения или исключения | Список значений из `AnixartParser.get_genres()`, например `["комедия", "повседневность"]` |
| `profile_list_exclusions` | `None` | Списки профиля, релизы из которых нужно исключить | Список ID: `0` — избранное; `1` — смотрю; `2` — в планах; `3` — просмотрено; `4` — отложено; `5` — брошено |
| `types` | `None` | Озвучки и субтитры | Список положительных ID типов из Anixart `GET /type/all`, например `[9]` для AnimeVost на момент написания документации |
| `age_ratings` | `None` | Возрастные рейтинги | Список ID: `1` — 0+; `2` — 6+; `3` — 12+; `4` — 16+; `5` — 18+ |

`None` отключает соответствующий фильтр. Для списочных параметров `None` сериализуется как новый пустой список, поэтому переданные списки не изменяются.
...
4. Получить список аниме с шикимори по фильтрам
    ...
    ```
    Доступные фильтры:
    - status - текущие статусы выхода (указывается строками в списке) (по умолчанию пусто (не учитывается в фильтрах)). Для исключения статуса из выдачи нужно указать перед ним восклицательный знак (!статус)
        <details>
    ...
    - anime_type - типы аниме (указывается строками в списке) (по умолчанию пусто (не учитывается в фильтрах)). Для исключения типа из выдачи нужно указать перед ним восклицательный знак (!тип)
        <details>
        <summary>Список доступных типов</summary>
        
        - tv - ТВ Сериал
            - tv_13 - ТВ Сериал 12 серий (да, там tv_13, не опечатка, хз почему)
            - tv_24 - ТВ Сериал 24 серии
            - tv_48 - ТВ Сериал 48+ серий
        - movie - Фильм
        - ova - OVA
        - ona - ONA
        - special - спецвыпуск
    ...
    - rating - возрастной рейтинг (можно указать несколько значений в списке или одно строкой) (по умолчанию пустой список (не учитывается в фильтрах)). Для исключения возрастного рейтинга из выдачи нужно указать перед ним восклицательный знак (!рейтинг)
        <details>
    ...
    - season - Сезон и год (указывается строками в списке) (по умолчанию пустой список (не учитывается в фильтрах)). Для исключения сезона из выдачи нужно указать перед ним восклицательный знак (!сезон)
        ...
        * 70-е - `197x`
        * До 1980 - `ancient`
    - duration - длительность серии (можно указать несколько значений в списке или одно строкой) (по умолчанию пустой список (не учитывается в фильтрах)). Для исключения длительности серии из выдачи нужно указать перед ним восклицательный знак (!рейтинг)
        <details>
        <summary>Список доступных длительностей</summary>
    ...
    - origin - первоисточник (указывается строками в списке) (по умолчанию пустой список (не учитывается в фильтрах)). Для исключения первоисточника из выдачи нужно указать перед ним восклицательный знак (!источник)
        Можно указывать как английский, так и русский вариант.
        Список доступных первоисточников можно получить в переменных origins_list и origins_list_ru (индексы первоисточников из списка origins_list_ru соответствуют английским вариантам в списке origins_list)
        
        <details>
        <summary>Список доступных первоисточников</summary>
        Английский:
        ```json
        ["original", "manga", "web_manga", "four_koma_manga", "light_novel", "novel", "web_novel", "visual_novel", "game", "card_game", "book", "picture_book", "radio", "music", "mixed_media", "other"]
        ```
        Русский:
        ```json
        ["Оригинал", "Манга", "Веб-манга", "Ёнкома", "Ранобэ", "Роман", "Веб-новелла", "Визуальная новелла", "Игра", "Карточная игра", "Книга", "Книга с картинками", "Радио", "Музыка", "Более одного", "Другое"]
        ```
        </details>
    - studios - студии анимации (указывается строками в списке) (по умолчанию пустой список (не учитывается в фильтрах)). Для исключения первоисточника из выдачи нужно указать перед ним восклицательный знак (!студия)
        [!] Отсутствует валидация данных.
        Посмотреть актуальный список можно на шикимори или воспользоваться списком ниже (возможно не полный).
        Для корректной работы следует указывать полную строку (с числами).
        <details>
        <summary>Список студий (может быть не актуальным)</summary>
        ```json
        ["1-Studio-Pierrot", "10-Production-I.G", "103-Tatsunoko-Production", "11-Madhouse", "111-NHK", "112-Brain's-Base", "13-Studio-4°C", "14-Sunrise", "150-Sanrio", "18-Toei-Animation", "2-Kyoto-Animation", "200-Tezuka-Productions", "207-Magic-Bus", "218-Zexcs", "22-Nippon-Animation", "242-Ashi-Productions", "247-Shin-Ei-Animation", "27-Xebec", "28-OLM", "291-CoMix-Wave-Films", "3-Gonzo", "300-SILVER-LINK.", "36-Gallop", "37-Studio-Deen", "38-Arms", "4-Bones", "41-Satelight", "44-Shaft", "48-AIC", "56-A-1-Pictures", "569-MAPPA", "6-Gainax", "65-Tokyo-Movie-Shinsha", "68-Mushi-Production", "7-J.C.Staff", "73-TMS-Entertainment", "858-Wit-Studio", "86-Group-TAC", "91-feel.", "95-Doga-Kobo"]
        ```
        </details>
    - licensed - лицензия (указывается строками в списке) (по умолчанию пустой список (не учитывается в фильтрах)). Для исключения лицензии из выдачи нужно указать перед ней восклицательный знак (!лицензия)
        [!] Отсутствует валидация данных.
        Посмотреть актуальный список можно на шикимори или воспользоваться списком ниже (возможно не полный).
        Для корректной работы следует указывать полную строку (плюсы заменяют пробелы).
        <details>
        <summary>Список студий (может быть не актуальным)</summary>
        ```json
        ["any", "Crunchyroll", "Wakanim", "DEEP", "Netflix", "2x2", "FAN", "MC+Entertainment", "VK", "Амедиатека", "CLS+Media", "Русский+Репортаж", "Reanimedia", "Ракета+Релизинг", "Иви", "Пионер", "Мега-Аниме", "Кинологистика", "RUSCICO", "Экспонента", "Cinema+Prestige", "XL+Media", "RWV", "Вольга", "Первый+ТВЧ", "Централ+Партнершип"]
        ```
        </details>
    - rating_from - оценка на Шикимори от включительнно (целое число от 0 до 10) (по умолчанию отсутствует (не учитывается в фильтрах))
    - rating_to - оценка на Шикимори до включительно (целое число от 0 до 10) (по умолчанию отсутствует (не учитывается в фильтрах))
    - start_page - начальная страница (начиная с 1) (по умолчанию 1)
    - page_limit - какое количество страниц парсить (по умолчанию 3)
    - sort_by - тип сортировки (по умолчанию rating) (одно значение строкой)
        <details>

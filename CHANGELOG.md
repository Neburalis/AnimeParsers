...
## Unreleased
- Добавлен синхронный `AnixartParser` для поиска релизов, чтения каталога эпизодов и получения ссылок на внешние плееры.
- В `AnixartParser` добавлен анонимный фильтр релизов с полным набором параметров, проверкой ответа и данными пагинации.
- В `AnixartParser` добавлены `get_studios()` и `get_genres()` со снимком допустимых значений фильтра из официальных ресурсов Anixart 8.5.2.
- В `AnixartParser` добавлены расписание, связанные и случайные релизы, а также расширенный поиск со сведениями о группе релизов.
- Добавлены детерминированные offline-тесты API-контракта Anixart.
...
## To 1.17.0
- Исправлены ошибки в парсере shikimori при парсинге anime_info (возвращались разные варианты страниц)
- Заменен основной домен в парсере Shikimori на "shikimori.io"
- Добавлена возможность исключать параметры из выдачи anime_list на shikimori (указывая ! перед параметром)
- Добавлена возможность указывать русские варианты жанров и тем при фильтрации списка из выдачи anime_list на shikimori
- Добавлены все остальные параметры фильтров для anime_list ("Сезон и год", "Длительность серии", "Первоисточник", "Студия", "Лицензия", "Рейтинг от", "Рейтинг до")
- Исправлены ошибки в тестах (shikimori заменили "TV Сериал" на "ТВ Сериал", тесты падали)
## To 1.18.0
- Добавлен враппер для апи Anilibria(Aniliberty) (синхронный)
- Исправлена ошибка в функции get_translations для апи кодика
